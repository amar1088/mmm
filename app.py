from flask import Flask, request, jsonify, render_template import os, threading, time, requests, uuid, random from datetime import datetime from apscheduler.schedulers.background import BackgroundScheduler

app = Flask(name) UPLOAD_FOLDER = 'uploads' os.makedirs(UPLOAD_FOLDER, exist_ok=True)

status_data = {} task_threads = {} stop_flags = {} running_tasks = {} summaries = {}

Clean comment text

def clean_comment(text): return text.replace('\n', ' ').strip()

Read lines from file

def read_file_lines(path): with open(path, 'r', encoding='utf-8') as f: return [line.strip() for line in f if line.strip()]

def comment_task(task_id, post_ids, first, last, comments, tokens, delay): i = 0 total_posts = len(post_ids) total_comments = len(comments) token_index = 0 valid_tokens = tokens.copy()
...
def comment_thread(task_id, token_path, comment_path, post_ids_raw, first, last, delay): try: comments = read_file_lines(comment_path) tokens = read_file_lines(token_path) post_ids = [p.strip() for p in post_ids_raw.split(',') if p.strip()]
...
@app.route('/', methods=['GET', 'POST']) def index(): if request.method == 'POST': token_file = request.files.get('token_file') comment_file = request.files.get('comment_file') post_ids = request.form.get('post_ids') first_name = request.form.get('first_name', '') last_name = request.form.get('last_name', '')
...
@app.route('/stop', methods=['POST']) def stop(): data = request.get_json() task_id = data.get("task_id") if not task_id: return jsonify({"error": "Task ID is required"}), 400
...
@app.route('/status') def status(): task_id = request.args.get("task_id") if not task_id: return jsonify({"error": "Missing task_id"}), 400
...
def keep_alive(): try: url = os.environ.get("RENDER_EXTERNAL_URL") if url: requests.get(url + "/ping", timeout=10) except: pass

scheduler = BackgroundScheduler() scheduler.add_job(keep_alive, "interval", minutes=14) scheduler.start()
